layui.use('table', function () {
    ...
    table.on('tool(test)', function (obj) {
        ...
        if (obj.event === 'del') {
            ...
            $("#checkSeenewsId").val(data.newsId);
            $("#checkSeeNewTitle").val(data.newTitle);
            $("#checkSeeNewsTime").val(data.newsTime);
            if(data.newsState==5){
                $("#checkSeeNewsState").val("启用");
            }else{
                $("#checkSeeNewsState").val("禁用");
            }
            layer.open({
                //layer提供了5种层类型。可传入的值有：0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
                type: 1,
...
layui.use(['form', 'layer', 'table'], function () {
    ...
    $("#register").click(function (data) {
        ...
        var addNewsTime = $("#addNewsTime").val();
        if (addNewsTitle == null || addNewsTitle == ''||addNewsTime == null || addNewsTime == '') {
            return;
        }
        //获取表单数据
        var jsonStr = JSON.stringify({
            "newTitle": $("#addNewsTitle").val(),
            "newsTime": $("#addNewsTime").val(),
        });
        //向后台发送数据
        $.ajax({
            contentType: "application/json",
